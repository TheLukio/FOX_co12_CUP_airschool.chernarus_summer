...
// disable group switching CORE 2.2 changes
...
(uniformContainer player) addItemCargoGlobal ["ACE_CableTie",2];
(uniformContainer player) addItemCargoGlobal ["ACE_Flashlight_XL50",1];
(uniformContainer player) addItemCargoGlobal ["ACE_MapTools",1];
(uniformContainer player) addItemCargoGlobal ["ACE_IR_Strobe_Item",1];
(vestContainer  player) addItemCargoGlobal ["ACE_fieldDressing",6];
(vestContainer  player) addItemCargoGlobal ["ACE_morphine",2];
(vestContainer  player) addItemCargoGlobal ["ACE_tourniquet",2];
(vestContainer  player) addItemCargoGlobal ["ACE_salineIV_250", 1];
(vestContainer player) addItemCargoGlobal ["ACE_SpraypaintBlue",1];
...
if (_typeofUnit == "m" || (player getUnitTrait "medic")) then {
	...
	if ((missionNamespace getVariable ["ace_medical_treatment_locationPAK", 0]) != 4) then { (unitBackpack player) addItemCargoGlobal ["ACE_personalAidKit", 1]; }; // PAKs
	...
	if (missionNamespace getVariable ["ace_medical_treatment_advancedBandages", 0] == 1) then {
		...
		(unitBackpack player) addItemCargoGlobal ["ACE_fieldDressing", 20];
		(unitBackpack player) addItemCargoGlobal ["ACE_bloodIV_250", 4];
		(unitBackpack player) addItemCargoGlobal ["ACE_bloodIV_500", 4];
		(unitBackpack player) addItemCargoGlobal ["ACE_bloodIV", 4];
		(unitBackpack player) addItemCargoGlobal ["ACE_plasmaIV_250", 2];
		(unitBackpack player) addItemCargoGlobal ["ACE_plasmaIV_500", 2];
		(unitBackpack player) addItemCargoGlobal ["ACE_plasmaIV", 2];
	};
};
...
if (_typeofUnit == "eng" || (player getUnitTrait "explosiveSpecialist")) then {
	player setVariable ["ACE_IsEngineer",1];
...
if (_typeofUnit == "uav" || (player getUnitTrait "UAVHacker")) then {	
	(vestContainer player) addItemCargoGlobal ["ACE_UAVBattery", 4];
	player addWeapon "ACE_Vector";
};

if (_typeofUnit == "eng" || _typeofUnit == "engm") then {
	(unitBackpack player) addItemCargoGlobal ["ACE_wirecutter", 1];
	(unitBackpack player) addItemCargoGlobal ["ACE_EntrenchingTool", 1]; // commented for Foxhound default loadouts
};
