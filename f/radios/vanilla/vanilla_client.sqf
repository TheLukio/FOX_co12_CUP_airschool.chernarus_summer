...
f_fnc_radioSwitchChannel = {
	...
	if _doAdd then {
		if !(setCurrentChannel (_id + 5)) then {
			...
			if (setCurrentChannel (_id + 5)) then {
				titleText [format["<t color='#FFBA26' size='1.5'>%1 </t><t size='1.5'>Connected %2</t>", _name, _reason], "PLAIN DOWN", -1, true, true];
...
private _radioText = "<br/><font size='18' color='#FFBA26'>RADIO CHANNELS</font>";
...
_radioText = _radioText + "<br/><br/><font size='18' color='#FFBA26'>CHANNEL LIST</font>";
...
if (f_param_radioMode == 1) then {
	_radioText = _radioText + "<br/><br/><font size='18' color='#FFBA26'>LOST RADIO</font><br/>Anyone may request a <execute expression=""[player, true] spawn f_fnc_giveRadioBackpack; hintSilent format['Radio assigned to %1', name player];"">Replacement Radio</execute> if your Radio Operator is MIA or any other technical issue.";
};
